const Aboutdata=require("../Model/About")
...
const SaveData=async(req,res)=>{
    ...
    req.body.image=filename; //a
        ...
        const response=await  Aboutdata.create(req.body);
...
const GetAboutData=async(req,res)=>{
    try{
        const response=await Aboutdata.find({});
        if(!response)
...
const UpdateAboutData = async (req, res) => {
    ...
    const data = req.body;
      ...
      const response = await Aboutdata.findByIdAndUpdate(
        id,data
...
const DeleteAboutData=async(req,res)=>{
    const id=req.params.id;
    try{
        const response=await Aboutdata.findByIdAndDelete(id);
        if(!response)
            return res.status(404).json({message:"About data not found"})
